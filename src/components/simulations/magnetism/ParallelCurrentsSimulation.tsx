...
import { RotateCcw, Sparkles, Download, Plus, Trash2, Rotate3d } from 'lucide-react';
...
export function ParallelCurrentsSimulation({ lang = 'en' }: { lang?: 'en' | 'si' | 'ta' }) {
  ...
  const logReading = () => {
    ...
    const typeLabel = interactionType === 'attract' ? t.attraction : interactionType === 'repel' ? t.repulsion : t.noForce;
    const newLog: TrialLog = {
      ...
      i2,
      distance,
      forcePerLength: parseFloat((forcePerLength * 1e6).toFixed(3)), // in uN/m
      type: typeLabel
    };
    setLogs([newLog, ...logs]);
  };
  ...
  const handleExportPDF = () => {
    const content = [
      `A/L Physics Laboratory - Force Between Parallel Conductors Report`,
      `Institution: Physics by Senath\n`,
      `Formula: F/L = μ₀I₁I₂ / (2πd)\n`,
      `Logged Parameters History:`,
      ...logs.map(log => 
        `[${log.timestamp}] I₁ = ${log.i1} A | I₂ = ${log.i2} A | Distance: ${log.distance} mm => F/L: ${log.forcePerLength} μN/m (${log.type})`
      ),
      `\nLab Observations Journal:\n${notes || 'No observations logged.'}`
    ].join('\n');
    downloadReportAsPDF('Parallel_Currents_Force_Report', {}, [], content);
  };
                ...
                {logs.map((log) => (
                  <div key={log.id} className="border border-slate-100 rounded-lg p-2.5 bg-slate-50/50 flex flex-col sm:flex-row justify-between sm:items-center gap-2 text-[10px]">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <span className="font-extrabold text-slate-800">I₁ = {log.i1} A, I₂ = {log.i2} A</span>
                        <span className="text-slate-400">|</span>
                        <span className="font-bold text-slate-650">d = {log.distance} mm</span>
                      </div>
                    ...
                    <div className="flex items-center gap-2.5">
                      <span className="font-mono font-extrabold text-blue-600">F/L = {log.forcePerLength} μN/m</span>
                    </div>
                  </div>
                ))}
              </div>
